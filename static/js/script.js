...
function copyMainCode(elementId, btnElement, type, codeId) {
    ...
    interactCode(type, codeId, 'view', null);
}
...
function copyRefLink() {
    const link = document.getElementById('my-ref-link').value;
...
async function viewProfile(email) {
    try {
        ...
        if(res.ok) {
            ...
            document.getElementById('pub-prof-img').src = data.photo;
            document.getElementById('pub-prof-name').innerText = data.name;
...
async function interactCode(type, id, action, btnElement) {
    if(action === 'like' && !isLoggedIn) { alert("Please login to like codes!"); switchAuthPage(); return; }
    try {
        const res = await fetch('/api/interact-code', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type, id, action }) });
        const data = await res.json();
        if(res.ok && action === 'like') { btnElement.classList.add('liked'); btnElement.innerHTML = `❤️ ${data.likes}`; }
    } catch(e) {}
}

async function handleRegistration(e) {
    e.preventDefault(); const savedRef = localStorage.getItem('refCode');
    try {
        const res = await fetch('/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: document.getElementById('reg-name').value, email: document.getElementById('reg-email').value, password: document.getElementById('reg-password').value, ref_code: savedRef }) });
        const data = await res.json();
        if (res.ok) { localStorage.removeItem('refCode'); alert(data.message); setAuthMode('normal'); } else { alert("Error: " + data.message); }
    } catch (err) {}
}
...
async function handleLogin(e) {
    ...
    try {
        ...
        const data = await res.json();
        if (res.ok) { isLoggedIn = true; isPremiumUser = data.is_premium; document.getElementById('nav-login').innerText = "Dashboard"; await loadUserProfile(); loadDynamicContent(); switchPage('profile'); } else { alert("Error: " + data.message); }
    } catch (err) {}
}
...
async function handleLogout() {
    ...
    document.getElementById('nav-login').innerText = "Account"; document.getElementById('login-form').reset();
    document.getElementById('nav-notifications').style.display = 'none'; document.getElementById('btn-staff-panel').style.display = 'none';
    ...
    loadDynamicContent(); switchPage('home');
}
...
async function loadUserProfile() {
    try {
        ...
        if (res.ok) {
            ...
            document.getElementById('my-ref-link').value = `${window.location.origin}/?ref=${user.ref_code}`;
...
async function requestPayout() {
    const amount = prompt("Enter amount to withdraw (Min ₹100):"); if(!amount) return;
    const upi = prompt("Enter your UPI ID to receive payment:"); if(!upi) return;
    try { const res = await fetch('/api/creator/payout', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({amount: parseInt(amount), upi: upi}) }); const data = await res.json(); if(res.ok) { alert("Payout Requested! Admin will verify."); loadUserProfile(); } else alert(data.error); } catch(e) {}
}

async function submitSupportTicket(e) {
    e.preventDefault(); const btn = e.target.querySelector('button'); btn.innerText = "Sending...";
    try { const res = await fetch('/api/tickets', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ subject: document.getElementById('tick-subject').value, message: document.getElementById('tick-msg').value }) }); if(res.ok) { alert("Ticket opened!"); e.target.reset(); loadMyTickets(); } else { alert("Error."); } } catch(e) {} btn.innerText = "Send to Support";
}
...
async function loadMyTickets() {
    try {
        ...
        if(res.ok) {
            const data = await res.json(); const container = document.getElementById('my-tickets-list');
            if(data.length === 0) { container.innerHTML = '<p style="color: #888; font-size: 0.9rem;">No tickets found.</p>'; return; }
            container.innerHTML = data.map(t => `<div style="background: rgba(0,0,0,0.5); padding: 15px; border-radius: 8px; border-left: 3px solid ${t.status==='Closed'?'#00ff88':'#ff5f56'}; margin-bottom: 10px;"><div style="display:flex; justify-content:space-between;"><strong style="color:#fff;">${t.subject}</strong><span style="color:${t.status==='Closed'?'#00ff88':'#ff5f56'}; font-size:0.8rem;">${t.status}</span></div><p style="color:#ccc; font-size:0.9rem; margin:5px 0;">${t.message}</p>${t.admin_reply ? `<div style="background: rgba(0, 210, 255, 0.1); padding: 10px; border-radius: 5px; margin-top: 10px; border-left: 2px solid #00d2ff;"><strong style="color: #00d2ff; font-size: 0.8rem;">Staff Reply:</strong><p style="color: #fff; font-size: 0.9rem; margin: 5px 0 0 0;">${t.admin_reply}</p></div>` : ''}</div>`).join('');
        }
    } catch(e) {}
}

async function submitCreatorCode(e) {
    e.preventDefault(); const btn = e.target.querySelector('button'); btn.innerText = "Submitting...";
    try { const res = await fetch('/api/creator/upload', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: document.getElementById('cr-title').value, category: document.getElementById('cr-cat').value, price: document.getElementById('cr-price').value, code: document.getElementById('cr-code').value }) }); if(res.ok) { alert("Code submitted for review!"); e.target.reset(); switchProfTab('locker'); } else { alert("Failed."); } } catch(e) {} btn.innerText = "Submit for Review";
}
...
async function loadMyPurchases() {
    try {
        ...
        if (res.ok) {
            ...
            if (isBannedUser) { container.innerHTML = '<div style="background: rgba(255, 95, 86, 0.1); border: 1px solid #ff5f56; padding: 15px; border-radius: 8px; text-align: center;"><p style="color: #ff5f56; font-size: 0.95rem; margin: 0; font-weight: bold;">🚫 Locked while restricted.</p></div>'; return; }
            ...
            if (data.codes.length === 0) { container.innerHTML = '<p style="color: #666; font-size: 0.9rem;">No unlocked files.</p>'; return; }
...
async function loadLeaderboard() {
    try {
        ...
        if(res.ok) {
            ...
            container.innerHTML = data.map((c, i) => `<div style="display: flex; justify-content: space-between; padding: 15px; border-bottom: 1px solid #333; align-items: center;"><div><span style="color: ${i===0?'#f5af19':i===1?'#ccc':i===2?'#cd7f32':'#888'}; font-weight: bold; font-size: 1.2rem; margin-right: 15px;">#${i+1}</span><a class="creator-link" onclick="viewProfile('${c.email}')">${c.name}</a></div><span style="color: #ff5f56; font-weight: bold;">❤️ ${c.score} Score</span></div>`).join('');
        }
    } catch(e) {}
}
    ...
    try {
        ...
        const generateCodeHTML = (codes, isPremiumSection = false, typeName) => {
            ...
            return codes.map((item, index) => {
                ...
                else { html += `<div class="code-container" style="${blurStyle}"><div class="code-header"><div class="dots"><div class="dot red"></div><div class="dot yellow"></div><div class="dot green"></div></div><div><button class="copy-main-btn" style="background: transparent; color: #00ff88; border: 1px solid #00ff88; margin-right: 10px;" onclick="runSandbox('code-${item.id}')">▶ Run Preview</button><button class="copy-main-btn" style="background: ${isPremiumSection ? '#f5af19' : ''}; color: ${isPremiumSection ? '#000' : ''};" onclick="copyMainCode('code-${item.id}', this, '${typeName}', ${item.id})">Copy Script</button></div></div><pre id="code-${item.id}">${item.code}</pre></div>`; }
                
                html += `<div class="social-proof" style="${isLocked ? 'filter: blur(5px); pointer-events: none;' : ''}"><div>👁️ ${item.views} Views | By <a class="creator-link" onclick="viewProfile('${item.creator_email}')">${item.creator}</a></div><button class="like-btn" onclick="interactCode('${typeName}', ${item.id}, 'like', this)">🤍 ${item.likes}</button></div>`;
