...
function copyMainCode(elementId, btn) {
    ...
    setTimeout(() => { btn.innerText = 'Copy Script'; btn.style.background = originalColor || '#00d2ff'; }, 2000);
}
...
function copyPrompt(btn, text) {
    ...
    setTimeout(() => { btn.innerText = original; btn.classList.remove('copied'); }, 2000);
}
...
function toggleFlipCard() {
    ...
    mainTitle.innerHTML = isFlipped ? `Create <span style="color:#00d2ff;">Account</span>` : `Access <span style="color:#00d2ff;">Account</span>`;
}

function setAuthMode(mode) {
    const toggle = document.getElementById('auth-toggle'), btnN = document.getElementById('btn-normal'), btnP = document.getElementById('btn-premium'), span = document.getElementById('login-title-span'), flip = document.getElementById('flip-container'), lock = document.getElementById('premium-lock-overlay');
    if (mode === 'premium') {
        toggle.classList.add('premium-mode'); btnP.classList.add('active'); btnN.classList.remove('active'); span.style.color = '#f5af19'; flip.classList.add('blurred-locked'); lock.classList.add('show');
    } else {
        toggle.classList.remove('premium-mode'); btnN.classList.add('active'); btnP.classList.remove('active'); span.style.color = '#00d2ff'; flip.classList.remove('blurred-locked'); lock.classList.remove('show');
    }
}
