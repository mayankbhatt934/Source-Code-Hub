...
function copyPrompt(btn, text) { 
    ...
    setTimeout(() => { 
        ...
        btn.style.background = "rgba(176, 106, 179, 0.15)"; 
        btn.style.color = "#d49ce0"; 
        btn.style.borderColor = "#b06ab3";
    }, 2000); 
}
...
// NOTE: I COMPLETELY REMOVED THE 10-SECOND CONTENT REFRESH TIMER. 
// Your codes will NEVER auto-hide or randomly swap places again.
...
// Memory states to power the Accordion
...
async function loadDynamicContent() {
    try {
        ...
        // FUNCTION FOR FREE & PREMIUM CODES
        const generateCodeHTML = (codes, isPremiumSection = false, typeName) => {
            ...
            return codes.map((item, index) => {
                ...
                const actionBarHTML = `
                <div style="display: flex; justify-content: space-between; align-items: center; padding-top: 10px; color: #888; font-size: 1.15rem;">
                    <div style="display: flex; gap: 15px; align-items: center;">
                        <span title="Views" style="display: flex; align-items: center; gap: 5px; color: ${mainColor};"><i class="bx bx-show"></i> <span id="view-${typeName}-${item.id}" style="font-size:0.9rem; color: #aaa;">${item.views || 0}</span></span>
                        <span title="Like" onclick="toggleAction(${item.id}, '${typeName}', 'like')" style="cursor: pointer; display: flex; align-items: center; gap: 5px; transition: 0.3s;" onmouseover="this.style.color='#00ff88'" onmouseout="this.style.color=''"><i class="bx bx-like"></i> <span id="like-${typeName}-${item.id}" style="font-size:0.9rem; color: #aaa;">${item.likes || 0}</span></span>
                        <span title="Dislike" onclick="toggleAction(${item.id}, '${typeName}', 'dislike')" style="cursor: pointer; display: flex; align-items: center; gap: 5px; transition: 0.3s;" onmouseover="this.style.color='#ff5f56'" onmouseout="this.style.color=''"><i class="bx bx-dislike"></i> <span id="dislike-${typeName}-${item.id}" style="font-size:0.9rem; color: #aaa;">${item.dislikes || 0}</span></span>
                ...
                if (!isPremiumSection && !isFullWebsite) {
                    ...
                    <div class="code-wrapper" data-tags="${item.tags}" style="margin-bottom: 30px;" data-aos="fade-up">
                        <h3 style="color: #00d2ff; margin-bottom: 5px; font-size: 1.4rem;">${num}. ${item.title}</h3>
                    ...
                    <div class="code-wrapper" data-tags="${item.tags}" style="margin-bottom: 30px;" data-aos="fade-up">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 5px;">
                            <h3 style="color: ${mainColor}; margin: 0; font-size: 1.4rem;">${num}. ${item.title}</h3>
                        ...
                        <div style="background: #1e1e2e; border-radius: 10px; overflow: hidden; border: 1px solid rgba(245, 175, 25, 0.3); box-shadow: 0 10px 30px rgba(0,0,0,0.5); position: relative;">
                            <div style="background: #2a2a3b; padding: 10px 15px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(245, 175, 25, 0.1);">
                                ...
                                <div style="display: flex; gap: 10px; align-items: center;">
                                    <span style="color: #aaa; font-size: 0.85rem; display: flex; align-items: center; margin-right: 10px;">By <b style="color: #fff; margin-left: 4px;">${item.creator}</b></span>
                                    <button id="copy-premium-${item.id}" class="copy-reveal-btn-premium" onclick="copyMainCode('content-premium-${item.id}', this, 'premium', ${item.id});" style="display: ${!isLocked && isRevealed ? 'inline-block' : 'none'}; background: #f5af19; color: #000; border: none; padding: 5px 12px; border-radius: 5px; font-weight: bold; cursor: pointer; font-size: 0.85rem;">Copy Script</button>
                                </div>
                            </div>
        ...
        if(document.getElementById('free-single-content')) document.getElementById('free-single-content').innerHTML = generateCodeHTML(data.codes.filter(c => c.category === 'Single Page' || c.category === 'Single Page Code'), false, 'free');
        if(document.getElementById('free-full-content')) document.getElementById('free-full-content').innerHTML = generateCodeHTML(data.codes.filter(c => c.category.includes('Full Website')), false, 'free');
        if(document.getElementById('prem-single-content')) document.getElementById('prem-single-content').innerHTML = generateCodeHTML(data.premium_codes.filter(c => c.category === 'Single Page' || c.category === 'Single Page Code'), true, 'premium');
        if(document.getElementById('prem-full-content')) document.getElementById('prem-full-content').innerHTML = generateCodeHTML(data.premium_codes.filter(c => c.category.includes('Full Website')), true, 'premium');
