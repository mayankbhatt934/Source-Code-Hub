...
function filterCodes(type) { const query = document.getElementById(`search-${type}`).value.toLowerCase(); ['single', 'full'].forEach(tab => { document.querySelectorAll(`#${type}-${tab}-content .code-wrapper`).forEach(el => { el.style.display = el.querySelector('.code-title span').innerText.toLowerCase().includes(query) ? 'block' : 'none'; }); }); }
...
async function loadUserProfile() {
    try {
        ...
        if (res.ok) {
            ...
            } else { const banner = document.getElementById('ban-banner'); if(banner) banner.remove(); ['nav-home', 'nav-free', 'nav-premium', 'nav-pricing'].forEach(id => document.getElementById(id).style.display = 'block'); if(document.querySelector('.page-section.active').id === 'page-login') switchPage('profile'); }
